"""Regime-aware PCMCI via PELT changepoint detection.

Detects regime boundaries via PELT (ruptures), encodes regime as an integer
context column, then runs PCMCI on the augmented data. The regime variable
is constrained to be exogenous (nothing causes it).

This is a two-stage approach: detect regimes first, then discover causality.
Compare with rpcmci.py which jointly learns regimes + causal structure.
...
def discover(
    ...
    **kwargs,
    ...
    """Regime-aware PCMCI: PELT regime detection + CMIknn.
    ...
    try:
        ...
        # sig_blocklength=1: the regime integer has phi≈1 within regimes,
        # which causes NaN in automatic block length. Force simple permutations.
        ci_test = CMIknn(knn=5, sig_samples=500, sig_blocklength=1)
    except (ImportError, AttributeError):
        warnings.warn("CMIknn unavailable. Falling back to ParCorr for regime_pcmci.")
...
def _build_link_assumptions(
    ...
    """PCMCI link assumptions for regime-augmented graph.
